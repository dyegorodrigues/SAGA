...
## 📌 2026-07-19 (Claude — 31ª rodada): re-sincronização + Matrícula gentil + MODO AI-Studio-principal
- MATRÍCULA (placement) consertada: era escada FIXA sondando toda habilidade até nível 4
  (soma/subtração nível 4 massacravam o iniciante — bug reportado pelo Zeus no teste).
  Agora GENTIL: senso numérico sonda [2,3], operações só [1,2] visual. Nunca mais nível 4
  no iniciante. `src/utils/matricula.ts` + testes. 380 testes verdes.
- FLUXO NOVO (a pedido do Zeus): AI Studio vira o PRINCIPAL; Gemini alimenta a `main`;
  Claude vira arquiteto/coordenador que re-sincroniza pela `main`+este changelog. Regras
  em `docs/arquitetura/fluxo-multi-ia.md` §MODO ATUAL. GEMINI: mantenha este changelog no
  topo a cada mudança (data · o quê · porquê · arquivos) — é como o Claude sabe o que você fez.
- Triagem dos bugs que o Zeus reportou no teste (fila, ver CLAUDE.md 31ª): Matrícula ✅;
  a investigar/consertar: aulinha de contar/somar (numerais na ordem), elogio ainda longo,
  áudio travando, erros de português nas falas, "jogar de novo" na 1ª variação, Pular sem
  aulinha, Dojo inexistente (é a E5). Vários já tinham fix meu — confirmar se o AI Studio
  não reverteu ao "melhorar" sozinho.

**Status de Sincronização:** ✅ SINCRONIZADO pelo Claude em 2026-07-19 (30ª rodada), com ressalvas:
1. Árvore de docs ADOTADA na oficial (com correção de todas as referências).
2. Docs novos absorvidos (didatica/ completa, MAB, MEGA_ANALISE, skills-flow, ANALISE_ITS).
3. `.claude/` NÃO foi deletado — as skills de lá são ferramentas VIVAS do Claude Code (a cópia em docs/skills do zip era redundante e não foi trazida).
4. O CÓDIGO do zip NÃO foi mesclado: a base dele era a 23ª rodada e a oficial já está na 29ª (Aulinha, áudio redondo, Compositor, Matrícula, Dinheirinho novo). `PedagogicalEditor.tsx` preservado em auditorias_e_backups/externos/aistudio-2026-07-19 para cherry-pick futuro.
5. Veredito do MAB no `docs/arquitetura/parecer-auditorias-externas.md` (§MAB).

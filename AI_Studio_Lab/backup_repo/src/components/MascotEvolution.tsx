...
export const STAGES: EvolutionStage[] = [
  { stage: 1, name: "Ovo Mágico 🥚", desc: "Seu amiguinho ainda está dormindo no ovo mágico. Resolva desafios de matemática para fazê-lo chocar! 🌟", minStars: 0 },
  { stage: 2, name: "Mascote Bebê 👶", desc: "Que fofinho! Seu mascote nasceu e agora é um bebê curioso, pronto para aprender e brincar! 🌱", minStars: 15 },
  { stage: 3, name: "Jovem Aprendiz 👦", desc: "Incrível! Seu mascote já fica em pé e está muito esperto com números, somas e formas! 📚", minStars: 75 },
  { stage: 4, name: "Herói da Matemática 🦸", desc: "Uau! Ele ganhou superpoderes de cálculo mental e tem força total para resolver enigmas! ⚡", minStars: 250 },
  { stage: 5, name: "Mestre Lendário Supremo 👑", desc: "Magnífico! Seu mascote atingiu o ápice da sabedoria matemática e brilha como uma divindade cósmica! 💎✨", minStars: 700 },
];

export function getMascotStage(stars: number): EvolutionStage {
  if (stars >= 700) return STAGES[4];
  if (stars >= 250) return STAGES[3];
  if (stars >= 75) return STAGES[2];
  if (stars >= 15) return STAGES[1];
  return STAGES[0];
}

...
// Initialize Firestore using the safest, most compatible browser pattern (no persistent cache forcing inside iframe)
...
try {
  ...
  console.error("[Firestore] Failed to initialize Firestore with custom database ID:", e);
  try {
    firestoreInstance = initializeFirestore(app, { experimentalForceLongPolling: true });
  } catch (err) {
    console.error("[Firestore] Serious error initializing fallback Firestore instance:", err);
  }
}

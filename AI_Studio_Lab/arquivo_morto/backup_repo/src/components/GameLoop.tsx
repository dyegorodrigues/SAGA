...
export function GameLoop({
  ...
  // reset do sequenciamento a cada questão nova
  useEffect(() => {
    setOrderTaps([]);
    setOrderErr(false);
    setOrderShake(null);
  ...
  const startGuidedTutorial = () => {
    ...
    const play = () => {
      ...
      if (sound) speak(st.say, q.lang ? { lang: q.lang } : {});
      i++;
      setTimeout(play, st.ms ?? 2200);
    };
    play();
  };
  ...
  useEffect(() => {
    // no `journey` a própria viagem narra; o enunciado só é falado quando ela termina
    if (sound && !done && !status && q.kind !== "journey") {
      // "como fazer" só na 1ª questão da missão; depois vai direto ao enunciado
      speak(qSpeech(q, idx === 0), q.lang ? { lang: q.lang } : {});
    }
  }, [q, done]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handlePick = (val: any, forcedRight?: boolean) => {
    ...
    if (right) {
      ...
      if (idx >= WARMUP_QUESTIONS) {
        ...
        if (p.bad >= 2 && p.lvl > 1) {
          p.lvl--;
          p.bad = 0;
          currentToast = "Vamos treinar mais um pouco 💪";
        }
      }
    }
    ...
    // ⭐ XP vitalício: 1 por acerto + 5 de bônus por missão perfeita (nunca se gasta)
    ...
    const baseFb = right
      ? (p.streak >= 2 ? SHORT_OK[Math.floor(Math.random() * SHORT_OK.length)] : praises[Math.floor(Math.random() * praises.length)])
  ...
  const startGuidedCount = () => {
    ...
    const interval = setInterval(() => {
      ...
      if (current < q.n!) {
        ...
        const numWord = numbersPt[current] || String(current + 1);
        speak(numWord + "!");
        sfx.tick();
      } else {
        ...
        setTimeout(() => {
          ...
          const numLabel = numbersPlural[q.n!] || String(q.n);
          speak(`São ${numLabel} figuras no total! 🎉`);
        }, 800);
      }
    }, 1200);
  };
  ...
  const startGuidedTens = () => {
    if (guidedIdx !== null) return;
    sfx.level();
    setGuidedIdx(0); // 0 = showing tens
    const numTens = q.t || 0;
    const numUnits = q.u || 0;
    const isPluralT = numTens > 1;
    
    speak(`Aqui temos ${numTens} ${isPluralT ? 'dezenas' : 'dezena'}, que vale ${numTens * 10}!`);
    
    setTimeout(() => {
      setGuidedIdx(1); // 1 = showing units
      const isPluralU = numUnits !== 1;
      speak(`E mais ${numUnits} ${isPluralU ? 'unidades' : 'unidade'}.`);
      
      setTimeout(() => {
        setGuidedIdx(null);
        speak(`Tudo junto forma o número ${(numTens * 10) + numUnits}!`);
      }, 3000);
    }, 3500);
  };

  const startGuidedSubvis = () => {
    if (guidedIdx !== null) return;
    sfx.level();
    let current = 0;
    ...
    <div className="mk-pop text-left">
        ...
        <div className="mk-pop" style={{ background: C.card, borderRadius: 24, boxShadow: `0 6px 0 ${C.line}`, padding: "20px 14px", ...(q.kind === "order" || q.kind === "groups" ? { display: "none" } : {}) }}>
          ...
          {q.kind === "sum" && q.emoji && q.a != null && q.b != null && (
            <div className="flex flex-wrap items-center justify-center gap-3">
              <EmojiRow emoji={q.emoji} n={q.a} small />
              <BigText size={36}>+</BigText>
              <EmojiRow emoji={q.emoji} n={q.b} small />
              <BigText size={36}>= ?</BigText>
            </div>
          )}
            ...
            <div>
              <div className="flex flex-wrap justify-center gap-2">
                {Array.from({ length: q.a }).map((_, i) => {
                  ...
                  return (
                    <span key={i} style={{ position: "relative", fontSize: 34, opacity: gone ? 0.25 : 1 }}>
                      {q.emoji}
                      {gone && (
          ...
          {q.kind === "weather" && q.big && (
            <div className="flex justify-center"><WeatherScene type={q.big as Weather} /></div>
          )}

          {q.kind === "grow" && q.n != null && (
            <div className="flex justify-center"><GrowthScene stage={q.n as 1 | 2 | 3 | 4} /></div>
          )}

          {q.kind === "daypart" && q.big && (
            <div className="flex justify-center"><DayPartScene type={q.big as DayPart} /></div>
          )}

          {q.kind === "emotion" && q.big && (
            <div className="flex justify-center"><EmotionScene type={q.big as Emotion} /></div>
          )}

          {q.kind === "lifestage" && q.n != null && (
            <div className="flex justify-center"><PersonLifeScene stage={q.n as 1 | 2 | 3 | 4} /></div>
          )}

          {q.kind === "animal" && q.n != null && (
            <div className="flex justify-center"><AnimalLifeScene stage={q.n as 1 | 2 | 3 | 4} /></div>
          )}
          ...
          {q.kind === "tenframe" && q.n != null && (
            <TenFrame filled={q.n} filled2={q.big === "add" ? q.u ?? null : null} />
          )}
      ...
      <div className="mt-5">
        {q.kind === "journey" && !journeyDone ? (
          ...
          <div className={`grid gap-3.5 ${q.options.length === 2 ? "grid-cols-2" : "grid-cols-2"}`}>
            {q.options.map((o, i) => {
              ...
              return (
                ...
                >
                  {/* Opção audível (GraphoGame): 🔊 pra criança OUVIR e escolher por som */}
                  {q.audibleOptions && !status && (
                    <span
                      ...
                      className="absolute top-1 right-1 w-6 h-6 flex items-center justify-center rounded-full bg-white/70 text-xs active:scale-90"
                    >
                      🔊
                    </span>
                  )}

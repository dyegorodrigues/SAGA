# Auditoria P20 — persistência local × nuvem

**Data:** 8/ago/2026  
**Branch:** `codex/integrar-bloco-f0`  
**Estado:** diagnóstico read-only; nenhuma regra de persistência foi alterada.

## 1. Inventário real do checkout

O inventário abaixo vem de `git ls-files` + `find`, não do índice de busca do conector.
...
## 2. Resolução TypeScript de `./services/storageSync`

```text
(trace não encontrou storageSync)
```

## 3. Referências a load/save/reconcile/revision/SAVE_REQUEST

```text
src/App.tsx:43:import { saveStateToCloud, loadStateFromCloud, getCurrentUserEmail, logoutUser, auth } from "./lib/firebase";
src/App.tsx:63:const nuvem = criarSincronizador({ gravar: saveStateToCloud });
src/App.tsx:303:          cloudState = await loadStateFromCloud();
src/lib/reconciliacaoDeSaves.test.ts:115:    const posNuvem = app.indexOf("await loadStateFromCloud()");
src/lib/firebase.ts:110:      const state = await loadStateFromCloud();
src/lib/firebase.ts:128:      const state = await loadStateFromCloud();
src/lib/firebase.ts:154:      const state = await loadStateFromCloud();
src/lib/firebase.ts:233:export async function saveStateToCloud(state: State): Promise<void> {
src/lib/firebase.ts:268:export async function loadStateFromCloud(): Promise<State | null> {
```
...
## 5. Perguntas que a análise seguinte deve fechar

- Qual payload vence quando `revision` é maior?
- O que acontece em empate de revisão com payload divergente?
- A reconciliação é campo-a-campo ou objeto inteiro?
- `dojoTracks`, `customTracks`, inventário, progresso, moedas, álbum e log viajam juntos?
- A migração ocorre antes de escolher o vencedor?
- O service worker pode reaplicar payload stale/equal revision?
- Existe proteção offline→online e teste de conflito?
- Qual é a unidade de atomicidade e quem incrementa `revision`?

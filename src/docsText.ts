...
5. **Errar ensina.** Todo erro previsto vira distrator, dica e microtutoria. Sem punição, sem sons agressivos, mascote jamais castiga (regra já vigente).
...
## §8. FEEDBACK E ERRO (a escada de 3 degraus)

Campos \`howto\` (COMO, falado na 1ª questão) e \`explain\` (PORQUÊ, falado ao errar) permanecem obrigatórios em todo gerador. A escada por questão:

1. **1º erro** → dica de estratégia falada (\`explain\` aponta o caminho, nunca a resposta) + esconder 1 opção absurda (redução de carga, regra já vigente).
2. **2º erro** → demonstração: o app resolve ESTE item animado (mini Mão Fantasma) e oferece um item gêmeo (mesmos params ±).
3. **Padrão de erro** (2+ erros do MESMO tipo de distrator na sessão) → o motor adaptativo registra a lacuna, injeta microtutoria do pré-requisito frágil na próxima sessão e agenda o item no banco de erros (resgate).

**Regras de tom (mantidas):** acerto = elogio curto ("Isso!"), mais curto ainda em streak; elogia esforço/estratégia, não "gênio"; transição rápida (250ms); erro nunca tem som agressivo; energia/mascote jamais punem.

**Distratores são pedagogia:** cada opção errada deve ser um erro típico do Grafo (ex.: 42−38 oferece 16, o "menor do maior"; 1/4+2/4 oferece 3/8). Distrator aleatório é desperdício de diagnóstico — o distrator escolhido REVELA o que a criança pensou (e alimenta o §8.3).
...
4. **Dojo:** 1 bloco por sessão da FD mais avançada destravada; alterna com a FD mais "enferrujada".
...
**Faixas (F0–F4):** idade é *referência*, não trava. A criança navega pelo grafo por pré-requisito, não por aniversário. Um filho de 6 pode estar em F2 numa strand e F0 em outra.
...
**Pré-req:** AL.01(b). **Kinds:** pattern (existente).
**Micros:**
...
O Dojo (rapid-fire) não é uma strand — é o modo de automatizar fatos JÁ compreendidos. Cada trilha destrava quando a competência-mãe atinge nível 4.
...
**Regra sagrada do Dojo:** velocidade só sobre compreensão. Fluência antes do conceito cria "papagaios de tabuada" — o oposto do SAGA.

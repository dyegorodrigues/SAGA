...
describe("opções nunca duplicadas NO VISUAL (label) — blindagem da auditoria externa", () => {
  // A auditoria descreveu o bug clássico: duas opções ESCRITAS iguais (ex.: GA e GA)
  // com values distintos — a criança clica na "certa" e é punida. Nosso teste de
  // value não pegaria; este pega pelo que a criança VÊ.
  for (const track of ALL) {
    for (const lvl of [1, 2, 3, 4, 5]) {
      it(`${track.id} nível ${lvl}: labels únicos em 80 questões`, () => {
        for (let i = 0; i < 80; i++) {
          const q = track.gen(lvl);
          const labels = q.options.map((o) => String(o.label ?? o.value).toUpperCase().trim());
          expect(new Set(labels).size, `label duplicado: ${JSON.stringify(q)}`).toBe(labels.length);
        }
      });
    }
  }

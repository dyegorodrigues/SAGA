      ...
      <div className="flex w-full max-w-[340px] flex-col items-center gap-3 select-none" data-material-dourado-stage data-material-modo={spec.modo}>
        {leitura && spec.exigeTroca && !trocaCompleta && (
          <div className="flex w-full flex-col gap-3">
            <div className={`rounded-2xl border-2 border-dashed p-3 ${mostrar?.destacarUnidades ? "border-amber-500 bg-amber-50" : "border-slate-200 bg-slate-50"}`} aria-label={`${10 - agrupadas} unidades ainda soltas`}>
              <div className="flex flex-wrap justify-center gap-2">
                {Array.from({ length: 10 - agrupadas }, (_, i) => (
                  <button type="button" key={`solta-${i}`} data-material-unidade-solta aria-label={`agrupar cubinho ${agrupadas + 1}`} disabled={travado} onClick={agruparUma} className="flex h-11 w-11 items-center justify-center rounded-xl border border-amber-200 bg-white shadow-sm disabled:cursor-default">
                    <MaterialUnitCube />
                  </button>
                ))}
              </div>
            ...
            <div data-material-alvo-troca data-material-tenframe className={`min-h-24 rounded-2xl border-2 border-dashed p-3 text-center ${mostrar?.destacarAlvoTroca ? "border-blue-500 bg-blue-50" : "border-blue-200 bg-white"}`}>
              <div className="mb-1 text-xs font-black uppercase tracking-wide text-blue-700">Junte 10 na moldura</div>
              <TenFrame moldura={{ casas: 10, ocupadas: Array.from({ length: agrupadas }, (_, i) => i), emoji: "🟨" }} />
              <div className="mt-1 text-xs font-bold text-slate-600">{agrupadas}/10</div>
            </div>
          </div>
        )}
        ...
        {leitura && trocaCompleta && (
          <>
            {spec.exigeTroca && (
              <motion.div data-material-equivalencia initial={{ opacity: 0, scale: 0.9 }} animate={{ opacity: 1, scale: mostrar?.pulsarEquivalencia ? [1, 1.06, 1] : 1 }} className="rounded-full bg-amber-100 px-4 py-2 text-sm font-black text-amber-900">
                {spec.equivalencia}
              </motion.div>
            )}
            ...
            <div className="grid w-full grid-cols-2 gap-2" aria-label="Escolha o número representado">
              {spec.alternativas.map(valor => (
                <button type="button" key={valor} data-material-resposta data-material-resposta-errada={valor !== spec.resposta || undefined} disabled={travado} onClick={() => responderLeitura(valor)} className="min-h-12 rounded-xl border-2 border-slate-200 bg-white px-3 py-2 text-lg font-black text-slate-800 shadow-sm active:translate-y-0.5 disabled:cursor-default">
                  {valor}
                </button>
              ))}
            </div>
          ...
          <>
            ...
            <div className="grid w-full grid-cols-2 gap-3">
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-3 text-center">
                ...
                <div className="mb-2 flex min-h-12 items-center justify-center">{dezenasProduzidas > 0 ? <MaterialTenBar /> : <span className="text-slate-400">0</span>}</div>
              ...
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-3 text-center">
                ...
                <div className="mb-2 flex min-h-12 items-center justify-center">{unidadesProduzidas > 0 ? <MaterialUnitCube /> : <span className="text-slate-400">0</span>}</div>
                <div className="flex justify-center gap-2">
            ...
            <button type="button" data-material-pronto disabled={travado} onClick={responderProducao} className="min-h-12 rounded-full bg-indigo-600 px-6 py-3 text-sm font-black text-white shadow-md active:translate-y-0.5 disabled:opacity-50">Pronto</button>
          </>
        )}
      </div>
    </PalcoEscalado>

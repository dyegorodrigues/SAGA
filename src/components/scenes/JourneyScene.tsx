...
export default function JourneyScene({ journey, sound, onDone }: Props) {
  ...
  // A cada parada: narra e avança SÓ quando a fala TERMINA (nunca por timer no meio
  // da voz — era isso que encavalava as narrações). Sem som: tempo de leitura.
  useEffect(() => {
    ...
    const goNext = () => {
      ...
      }, 1400); // respiro entre uma parada e outra
    };
    if (sound) speak(stop.say, { onEnd: goNext });
    else timer.current = window.setTimeout(() => {
      if (step < last) setStep((s) => s + 1);
      else doneRef.current();
    }, Math.max(2600, stop.say.length * 65));

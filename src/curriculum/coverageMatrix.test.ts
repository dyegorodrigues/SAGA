...
describe("Coverage Matrix executável", () => {
  it("preserva o fechamento P21.1 e deriva o baseline vigente por migrações nomeadas", () => {
    ...
    expect(COVERAGE_MIGRATIONS.at(-1)).toMatchObject({ id: "W36-GM.10", competence: "GM.10", delta: { composer: 1, fallback: -1, served: 1 } });
    expect(COVERAGE_BASELINE).toMatchObject({ composer: 61, legacy: 15, fallback: 14, served: 76, divergences: 11, modeSwaps: 12, toolIntroductions: 44 });
  });

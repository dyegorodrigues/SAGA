...
describe("Coverage Matrix executável", () => {
  it("preserva o fechamento P21.1 e deriva o baseline vigente por migrações nomeadas", () => {
    ...
    expect(COVERAGE_MIGRATIONS.at(-1)).toMatchObject({ id: "W40-GE.09", competence: "GE.09", delta: { composer: 1, fallback: -1, served: 1 } });
    expect(COVERAGE_BASELINE).toMatchObject({ composer: 65, legacy: 15, fallback: 10, served: 80, divergences: 11, modeSwaps: 12, toolIntroductions: 44 });
  });

...
describe("Coverage Matrix executável", () => {
  it("preserva o fechamento P21.1 e deriva o baseline vigente por migrações nomeadas", () => {
    ...
    expect(COVERAGE_MIGRATIONS.at(-1)).toMatchObject({ id: "W41-GE.10", competence: "GE.10", delta: { composer: 1, fallback: -1, served: 1 } });
    expect(COVERAGE_BASELINE).toMatchObject({ composer: 66, legacy: 15, fallback: 9, served: 81, divergences: 11, modeSwaps: 12, toolIntroductions: 44 });
  });

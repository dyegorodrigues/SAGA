...
describe("Coverage Matrix executável", () => {
  it("preserva o fechamento P21.1 e deriva o baseline vigente por migrações nomeadas", () => {
    ...
    expect(COVERAGE_MIGRATIONS.at(-1)).toMatchObject({ id: "W35-GM.06", competence: "GM.06", delta: { composer: 1, fallback: -1, served: 1 } });
    expect(COVERAGE_BASELINE).toMatchObject({ composer: 60, legacy: 15, fallback: 15, served: 75, divergences: 11, modeSwaps: 12, toolIntroductions: 44 });
  });

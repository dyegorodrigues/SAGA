...
describe("Coverage Matrix executável", () => {
  it("preserva o fechamento P21.1 e deriva o baseline vigente por migrações nomeadas", () => {
    ...
    expect(COVERAGE_MIGRATIONS.at(-1)).toMatchObject({ id: "W60-N2.04", competence: "N2.04", delta: { composer: 1, legacy: -1, divergences: -1 } });
    expect(COVERAGE_BASELINE).toMatchObject({ composer: 85, legacy: 5, fallback: 0, served: 90, divergences: 8, modeSwaps: 12, toolIntroductions: 44 });
  });

...
describe("Coverage Matrix executável", () => {
  it("preserva o fechamento P21.1 e deriva o baseline vigente por migrações nomeadas", () => {
    ...
    expect(COVERAGE_MIGRATIONS.at(-1)).toMatchObject({ id: "W37-N7.02", competence: "N7.02", delta: { composer: 1, fallback: -1, served: 1 } });
    expect(COVERAGE_BASELINE).toMatchObject({ composer: 62, legacy: 15, fallback: 13, served: 77, divergences: 11, modeSwaps: 12, toolIntroductions: 44 });
  });

    ...
    it("a superfície de resposta respeita o contrato de cada palco", () => {
      for (let lvl = 1; lvl <= 5; lvl += 1) {
        for (let i = 0; i < 40; i += 1) {
          ...
          // F15 responde no próprio EmojiRowRiscarStage: `q.options` é catálogo
          // diagnóstico para misconceptions; a superfície visível é o teclado
          // autoral inclusivo 0..tecladoAte. Não confundir os dois contratos.
          if (q.kind === "emojirow-riscar-f15") {
            ...
            expect(new Set(q.options.map(o => o.value)).size, `${id} L${lvl}: diagnósticos duplicados`)
              .toBe(q.options.length);
            expect(q.options.map(o => o.value), `${id} L${lvl}: diagnóstico sem gabarito`)
              .toContain(q.answer);
            continue;
          }
